...
async function fetchData() {
	let response = await fetch("https://maximilian-stark.developerakademie.net/defaultData.json");
...
function LoadPosts() {
	...
	for (index = 0; index < LoopData.length; index++) {
		...
		renderit(Posts, `<article><section class="mainContentImgsHead"><img src="${LoopDataUser[LoopData[index].PostedBy].profileimg}" alt="" srcset=""><p>${LoopDataUser[LoopData[index].PostedBy].userName}</p><p>${createtionDate}</p></section><div><img src="${LoopData[index].PostImg}" alt=""><div><div><a onclick="liken(${index});"><img id="likeimg${index}" src="./img/${isLike(LoopData[index].isLiked)}" alt=""></a><a onclick=""><img src="./img/message-2-64.png" alt=""></a><a onclick=""><img src="./img/add-64.png" alt=""></a></div><div><a onclick=""> <img src="./img/bookmark-4-64.png" alt=""></a></div></div></div><div class="mainContentComments"><h4 id="${"likeCounter" + index}">Liked ${LoopData[index].Likes} Times</h4><section><article id="commentsFromPost${index}">${comment}</article></section><div><input type="text" name="comment" id="commentin${index}"><input type="button" onclick="postComment(${index})" name="comment" value="Comment" id="commentBnt${index}"></div></div></article>`)
	}
}